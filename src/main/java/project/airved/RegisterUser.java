...
public class RegisterUser extends AppCompatActivity {
    ...
    private void getRegister() {
        ...
        if (cancel) {
            ...
            UserPasswordAuthProviderClient emailPassClient = Splash.client.getAuth().getProviderClient(UserPasswordAuthProviderClient.factory);
            emailPassClient.registerWithEmail(email, password).addOnCompleteListener(new OnCompleteListener<Void>() {
                @Override
                public void onComplete(@NonNull final Task<Void> task) {
                    if (task.isSuccessful()) {
                        Log.d("stitch-auth", "Successfully sent account confirmation email");
                        Toast.makeText(RegisterUser.this, "Verification Mail Sent.", Toast.LENGTH_SHORT).show();
                        startActivity(new Intent(RegisterUser.this, LoginActivity.class));
                    } else {
                        Log.e("stitch-auth", "Error registering new user:", task.getException());
